      ...
      <div className="p-4 space-y-8 container mx-auto px-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Lista de formularios de contacto recibidos</CardDescription>
            <p className="text-sm text-gray-600 mt-1">Total: <strong>{contacts.length}</strong></p>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow><TableHead>Fecha</TableHead><TableHead>Nombre</TableHead><TableHead>Email</TableHead><TableHead>Tipo Consulta</TableHead><TableHead>Mensaje</TableHead><TableHead>Acciones</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {contacts.length === 0 && (
                  <TableRow>
                ...
                {contacts.map((c) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell><Badge variant="outline">{c.typemessage}</Badge></TableCell>
                    <TableCell className="max-w-xs truncate" title={c.message ?? ""}>{c.message}</TableCell>
                    <TableCell><DeleteButtonContact id={c.id} /></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Lista de solicitudes de acceso anticipado recibidas</CardDescription>
            <p className="text-sm text-gray-600 mt-1">Total: <strong>{earlyAccess.length}</strong></p>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow><TableHead>Fecha</TableHead><TableHead>Nombre</TableHead><TableHead>Email</TableHead><TableHead>Teléfono</TableHead><TableHead>Ciudad</TableHead><TableHead>Tipo de piscina</TableHead><TableHead>Estado</TableHead><TableHead>Motivación</TableHead><TableHead>Acciones</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                ...
                {earlyAccess.map((req) => (
                  <TableRow key={req.id}>
                    <TableCell>{new Date(req.createdAt).toLocaleDateString("es-AR")}</TableCell>
